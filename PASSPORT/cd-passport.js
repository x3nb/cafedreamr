...
var CD = (function () {
  ...
  /* ---------------- the record ---------------- */
  function blank() {
    return { v: V, name: "", points: 0, tokens: 0, events: {}, quests: {}, doors: {}, badges: [], log: [], seen: {}, made: Date.now(), last: Date.now() };
  }
  function read() {
    try {
  ...
  /* tokens: the arcade's own money. points are progress, tokens are
     spendable — earned in miki's games room, spent at the claw machine
     and changed at the counter. they ride the record, so a balance is
     the same on every page.                                            */
  function tokenLog(n, label, accent, id) {
    state.log.push({ id: id, p: 0, t: Date.now(), label: label, accent: accent, group: "tokens" });
    if (state.log.length > 400) state.log = state.log.slice(state.log.length - 400);
  }
  function award(n, label, opts) {
    ...
    if (!n) return state.tokens || 0;
    state.tokens = (state.tokens || 0) + n;
    tokenLog(n, "+" + n + " tokens" + (label ? " \u00b7 " + label : ""), opts.accent || "#ccff00", opts.id || "token:earned");
    touch();
    if (!opts.quiet) toast("+" + n + "  " + (label || "tokens"), opts.accent || "#ccff00", opts.ms || 2400);
    announce();
    return state.tokens;
  }
  function spend(n, label, opts) {
    ...
    state.tokens -= n;
    tokenLog(n, "\u2212" + n + " tokens" + (label ? " \u00b7 " + label : ""), opts.accent || "#d8a24a", opts.id || "token:spent");
    touch();
    if (!opts.quiet) toast("\u2212" + n + "  " + (label || "tokens"), opts.accent || "#d8a24a", opts.ms || 2400);
    announce();
    return true;
  }
  ...
  function renderHUD(el) {
    ...
    el.innerHTML =
      ...
      ((state.tokens || 0) > 0 ? '<span class="cdh-tok">the arcade \u00b7 <b>' + state.tokens + "</b> tokens</span>" : "") +
      "</div>" +
  ...
  function merge(other) {
    ...
    if (other.tokens) state.tokens = Math.max(state.tokens || 0, other.tokens);
    recomputePoints();
  ...
  var api = {
    ...
    tokens: function () { return state ? (state.tokens || 0) : 0; },
